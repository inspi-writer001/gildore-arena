...
import { captureAndAnalyzeForDebug } from "@/lib/browser-session-runtime";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await captureAndAnalyzeForDebug({
    ...
    console.error("[debug/screenshots] failed", error);
    return NextResponse.json(
      {
        ok: false,
